...
import { expandDocument, expandEntity, expandMention, expandRelation } from './Queries'
...
export default class NetworkExpander extends React.Component<Props, State> {
  ...
  private handleExpandDocument = (helper: GraphHelper, documentId: string) => {
    expandDocument(this.context.pluginApi, this.props.datasetId, documentId).then(r => {
  ...
  private handleExpandMention = (helper: GraphHelper, mentionId: string) => {
    expandMention(this.context.pluginApi, this.props.datasetId, mentionId).then(r => {
  ...
  private handleExpandEntity = (helper: GraphHelper, entityId: string) => {
    expandEntity(this.context.pluginApi, this.props.datasetId, entityId).then(r => {
  ...
  private handleExpandRelation = (helper: GraphHelper, relationId: string) => {
    expandRelation(this.context.pluginApi, this.props.datasetId, relationId).then(r => {
  ...
  private addMention = (helper: GraphHelper, mention: BasicMentionNode): SigmaJs.Node => {
  ...
  private addEdge = (helper: GraphHelper, type: string, label: string, fromId: string, toId: string): SigmaJs.Edge => {
