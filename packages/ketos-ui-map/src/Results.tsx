...
import { DocumentSearchResultsView, SimpleMap } from 'ketos-components'
import { Circle, Layer, Map } from 'leaflet'
import * as React from 'react'
import { FeatureGroup, GeoJSON } from 'react-leaflet'
import { DrawableShape, EditControl, EditControlEvent } from 'react-leaflet-draw'
import { LocatedDocument, LocationEntity, Response } from './DataContainer'
...
class Results extends React.Component<Props, State> {
  ...
  render() {
    ...
    const features = results
      .map(d => {
        return (d.entities || []).filter(e => e.properties && e.properties.geoJson).map(e => {
          return (
            <GeoJSON
              key={d.id + '_' + e.id}
